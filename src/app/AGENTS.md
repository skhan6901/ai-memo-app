...
Next.js 15 App Router 기반 페이지 및 레이아웃 정의. 현재 단일 페이지(`/`) 구조이며, UI는 클라이언트 컴포넌트로 두고 데이터는 서버 액션으로 Supabase와 연동한다.

## 의존성 관계

- `@/components/*` — UI 컴포넌트 import
- `@/hooks/useMemos` — 메모 상태 관리 훅
- `@/app/actions/memos` — 메모 CRUD·요약 서버 액션
- `@/types/memo` — 메모 타입 정의

## Tech Stack & Constraints

- Next.js App Router (pages 디렉토리 아님)
- 메인 페이지는 `'use client'` 클라이언트 컴포넌트
- 데이터 영속화는 Supabase(Postgres), 서버 액션으로 접근

## 파일 구조

```
app/
├── actions/        # 서버 액션 (memos CRUD·요약)
├── favicon.ico     # 파비콘
...
2. 서버 액션에 민감한 키를 클라이언트에 노출하는 설계
